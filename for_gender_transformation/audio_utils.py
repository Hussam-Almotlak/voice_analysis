...
class DB_MelSpec(object):
    ...
    def __init__(self, sr=22050, ffts=1024, n_fft=1024, hop_t=0.010, win_t=0.025, n_mels=80, preemphasis=0.97, **kwargs):
        self.sr = sr
        self.n_fft = n_fft
        self.hop_t = hop_t
        self.win_t = win_t
    ...
    def __call__(self, tensor):
        ...
        tensor = to_melspec(tensor, sr=sr, n_fft=n_fft, hop_t=hop_t, win_t=win_t, log=False, n_mels=self.n_mels, preemphasis=preemphasis)
...
class DB_Spec(object):
    ...
    def __init__(self, sr=22050, n_fft=1024, hop_t=0.010, win_t=0.025, preemphasis=0.2, max_len=None, **kwargs):
        self.sr = sr
        self.n_fft = n_fft
        self.hop_t = hop_t
        self.win_t = win_t
    ...
    def __call__(self, tensor):
        ...
        win_t = self.win_t
        preemphasis = self.preemphasis
        # define the new sampling rate for downsampling
        new_sr = sr
        #tensor = librosa.resample(tensor, sr, new_sr)

        tensor = librosa.feature.melspectrogram(y=tensor, sr=new_sr, n_fft=2048, n_mels= 140)
        tensor = librosa.power_to_db(tensor, ref=np.max)
        tensor = torch.from_numpy(tensor).float()
