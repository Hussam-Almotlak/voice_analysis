...
class VAETrainer:
    ...
    def __init__(self, args, model, optimizer, train_loader, test_loader, test_dataset, writer):
        ...
        if self.loss_function == 'mi_loss':
            self.VAELosses = collections.namedtuple("Losses", ["loss", "reconstruction_nll", "prior_nll", "z_prediction_nll", "z_medium_nll", "z_global_entropy", "z_local_entropy", "z_medium_entropy"])
            ...
            self.VAELosses = collections.namedtuple("Losses", ["loss", "reconstruction_nll", "prior_nll", "z_prediction_nll", "z_medium_nll", "z_entropy", "z_global_entropy", "z_local_entropy", "z_medium_entropy"])
    ...
    def mi_loss(self, input, outputs, beta_kl=10., beta_mi=10., step=0):
        ...
        medium_sample = outputs.encoder_out.medium_sample
        ...
        z_medium_ll = torch.mean(outputs.medium_out.log_prob(medium_sample))
        ...
        z_medium_entropy = -torch.mean(outputs.encoder_out.medium_dist.log_prob(medium_sample))
        ...
        medium_term = z_medium_ll + z_medium_entropy

        Loss = - reconstruction_ll - beta_kl * KL_local_prior - beta_mi * MI_global_prediction - beta_mi * medium_term

        Losses = self.VAELosses(loss=Loss, reconstruction_nll=-reconstruction_ll, prior_nll=-prior_ll, z_prediction_nll=-z_prediction_ll, z_medium_nll=-z_medium_ll, z_global_entropy=z_global_entropy, z_local_entropy=z_local_entropy, z_medium_entropy=z_medium_entropy)
        return Losses
    ...
    def train(self, step):
        ...
        for batch_idx, (data, pers) in enumerate(self.train_loader):
            ...
            outputs = self.model(data, data, data, annealing = 0)
